...
The home page also supports category and publisher filters so visitors can narrow the catalog to titles that match their interests without leaving the page.
...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) displays repository issues in a Kanban board with Backlog, Ready, In progress, Blocked, and Done columns. Dragging an issue updates its `triage:*` label through the GitHub CLI and closes or reopens the issue when it enters or leaves Done. The canvas requires an authenticated `gh` CLI session.
